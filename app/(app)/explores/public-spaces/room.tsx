...
export default function PublicStarRoomPage() {
  ...
  useEffect(() => {
    ...
    if (active === "Foto's") {
      setPhotoIndex(0);
      api.get<MediaItem[]>(`${base}/photos`)
        .then(r => setPhotos(r.data))
        .catch(() => setPhotos([]));
    }
    if (active === "Video's") {
      setVideoIndex(0);
      api.get<MediaItem[]>(`${base}/videos`)
        .then(r => setVideos(r.data))
        .catch(() => setVideos([]));
    }
    if (active === "Audio") {
      setAudioIndex(0);
      api.get<MediaItem[]>(`${base}/audios`)
        .then(r => setAudios(r.data))
        .catch(() => setAudios([]));
    }
    if (active === "Documenten") {
      setDocIndex(0);
      api.get<DocumentItem[]>(`${base}/documents`)
        .then(r => setDocs(r.data))
        .catch(() => setDocs([]));
    }
    if (active === "Messages") {
      setMsgIndex(0);
      api.get<Message[]>(`${base}/messages`)
  ...
  if (!rooms?.length) return <EmptyOverlay message="No basic room available." />;

  const views = [
    { label: "Foto's"     as const,pos:[12, 9, -2], target: [1000, -9, -2] },
    { label: "Video's"    as const, pos:[14, 10, -2], target: [1000, -1000, -2] },
    { label: "Audio"      as const, pos:[10, 10, 7], target: [1000, -1000, 7]  },
    { label: "Documenten" as const, pos: [7, 5, -2], target: [9, 1, -4.5]  },
    { label: "Messages"   as const,  pos: [1.5, 10, 0], target: [1.5, -1000, 0] },
  ];
...
// Simple loading / error / empty overlays
const LoadingOverlay = () => <View style={styles.centered}><Text style={styles.info}>Loading…</Text></View>;
const ErrorOverlay   = ({message}:{message:string}) => <View style={styles.centered}><Text style={styles.error}>{message}</Text></View>;
const EmptyOverlay   = ({message}:{message:string}) => <View style={styles.centered}><Text style={styles.info}>{message}</Text></View>;
...
const styles = StyleSheet.create({
  ...
  documentWebView:{ flex:1, width:"100%", marginBottom:12 },
