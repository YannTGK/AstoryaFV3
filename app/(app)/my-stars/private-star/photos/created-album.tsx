...
export default function AlbumPage() {
  ...
  const uploadPhoto = async () => {
    ...
    if (!perm.granted) {
      Alert.alert("Permission required", "Enable photo access to upload."); return;
    }
  ...
  const deleteSelected = async () => {
    ...
    setSelected([]); setDeleteMode(false); fetchPhotos();
  };
  ...
  const onPressPhoto = (p: Photo, idx: number) => {
    if (deleteMode) {
      setSelected(s => s.includes(p._id) ? s.filter(i=>i!==p._id) : [...s, p._id]);
    } else {
  ...
  return (
    <View style={{ flex:1 }}>
        ...
        <View style={styles.albumTitleRow}>
          ...
          {deleteMode && (
            <TouchableOpacity style={styles.selectAllBtn}
              onPress={() =>
                setSelected(selected.length === images.length ? [] : images.map(p => p._id))
              }>
              <View style={[
        ...
        {menuOpen && (
          <View style={styles.menuBox}>
            ...
            {["Delete","Copy to album","Move to album"].map((action,idx)=>(
              <TouchableOpacity key={action} style={styles.menuItem}
                onPress={()=>{
                  setMenuOpen(false); setDeleteMode(true); setSelected([]);
                  setShowCopy(action==="Copy to album"); setShowMove(action==="Move to album");
                }}>
                <Feather name={["trash-2","copy","folder-minus"][idx] as any}
                         size={16} color="#11152A" style={{marginRight:10}}/>
      ...
      {showCopy && selected.length > 0 && (
        <TouchableOpacity style={styles.deleteBar}
          onPress={() => handleCopyOrMove("copy")}>
          <View style={styles.deleteBarBtn}>
      ...
      <View style={styles.plusWrapper}>
        <TouchableOpacity onPress={uploadPhoto}><PlusIcon width={50} height={50}/></TouchableOpacity>
      </View>
