...
export default function AddContentSpace() {
  ...
  const uploadFile = async (
    ...
    console.log("⏳ uploadFile gestart:", { uri, endpoint, fieldName });
    ...
    console.log("📄 Bestands-meta:", { filename, type });

    const form = new FormData();
    form.append(fieldName, { uri, name: filename, type } as any);

    // Bekijk de FormData entries (werkt enkel in debug)
    for (const pair of (form as any)._parts) {
      console.log("📦 FormData entry:", pair);
    }

    // Laat Axios de headers bepalen via interceptor
    const config = {
      // geen headers override
    };
    console.log("🚀 Versturen naar:", endpoint, "met config:", config);

    try {
      const res = await api.post(endpoint, form, config);
      console.log("✅ uploadFile response:", res.status, res.data);
      return res.data;
    } catch (err: any) {
  ...
  const handleDelete = async (
    type: "photos" | "videos" | "audios" | "documents" | "messages",
    id: string
  ) => {
    Alert.alert(
      "Verwijderen?",
      ...
      [
        { text: "Nee", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await api.delete(
                `/stars/${starId}/three-d-rooms/${roomId}/${type}/${id}`
              );
              // state updaten
              const setter = {
                photos: setPhotos,
  ...
  // pickAndUploadAudios met logs
  const pickAndUploadAudios = async () => {
    console.log("🎧 pickAndUploadAudios gestart");
    // multiple support aanzetten als je dat wilt
    const res = await DocumentPicker.getDocumentAsync({
      type: "audio/*",
      copyToCacheDirectory: false,
      multiple: false, // of true als je meerdere wilt
    });
    
    // Pak de eerste asset uit de array
    const asset = Array.isArray((res as any).assets)
      ? (res as any).assets[0]
    ...
    if (!uri) {
      console.log("❌ Geen URI gevonden op asset:", asset);
      return;
    }
    setLoading(true);
    try {
      const audio = await uploadFile(
        uri,
        `/stars/${starId}/three-d-rooms/${roomId}/audios/upload`,
        "audio"
      );
...
const pickAndUploadDocuments = async () => {
  ...
  // nieuwe API: assets-array, oude API: uri + type
  const cancelled = res.canceled ?? res.type === 'cancel';
  const uri =
    Array.isArray((res as any).assets) && (res as any).assets.length > 0
      ? (res as any).assets[0].uri
      : (res as any).uri;
  setLoading(true);
  try {
    // backend retourneert direct het gemaakte document-object
    const newDoc = await uploadFile(
      uri,
      `/stars/${starId}/three-d-rooms/${roomId}/documents/upload`,
      'document'
    );
      ...
      >
        ...
        <View style={styles.item}>
          ...
          <View style={styles.row}>
            {photos.map((p) => (
              <View key={p._id} style={styles.cardContainer}>
                <TouchableOpacity
                  onPress={() => openModal(p.url, "image")}
                >
                  <Image
                    source={{ uri: p.url }}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => handleDelete("photos", p._id)}
                >
            ...
            {photos.length < 10 && (
              <TouchableOpacity onPress={pickAndUploadPhotos}>
        ...
        <View style={styles.item}>
          ...
          <View style={styles.row}>
            {videos.map((v) => (
              <View key={v._id} style={styles.cardContainer}>
                <TouchableOpacity
                  onPress={() => openModal(v.url, "video")}
                >
                  <View style={styles.videoPlaceholder} />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.deleteBtn}
                  onPress={() => handleDelete("videos", v._id)}
                >
                  <Text style={styles.deleteTxt}>×</Text>
                </TouchableOpacity>
              </View>
            ...
            {videos.length < 3 && (
              <TouchableOpacity onPress={pickAndUploadVideos}>
                <PlusSquare width={60} height={60} />
              </TouchableOpacity>
            )}
          </View>
        ...
        <View style={styles.item}>
          ...
          <View style={styles.row}>
            {audios.map((a) => (
              <View key={a._id} style={styles.cardContainer}>
                <AudioIcon width={60} height={60} style={{ marginRight: 8 }} />
                <TouchableOpacity
                  style={styles.deleteBtn}
                  onPress={() => handleDelete("audios", a._id)}
                >
                  <Text style={styles.deleteTxt}>×</Text>
                </TouchableOpacity>
              </View>
            ...
            {audios.length < 3 && (
              <TouchableOpacity onPress={pickAndUploadAudios}>
        ...
        <View style={styles.item}>
          ...
          <View style={styles.row}>
            {documents.map((d) => (
              <View key={d._id} style={styles.cardContainer}>
                <PdfIcon width={60} height={60} />
                <TouchableOpacity
                  style={styles.deleteBtn}
                  onPress={() => handleDelete("documents", d._id)}
                >
                  <Text style={styles.deleteTxt}>×</Text>
                </TouchableOpacity>
              </View>
        ...
        {/* MESSAGES */}
        <View style={styles.item}>
          <Text style={styles.label}>Messages {messages.length}/5</Text>
          <View style={styles.row}>
            {messages.map((m) => (
              <View key={m._id} style={styles.cardContainer}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    router.push({
                      ...
                      params: { starId, roomId, msgId: m._id },
                    })
                  }
                >
                  ...
                  <Text style={styles.messageDate} numberOfLines={1}>
                    {new Date(m.addedAt).toLocaleTimeString("nl-BE", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </Text>
                ...
                <TouchableOpacity
                  style={styles.deleteBtn}
                  onPress={() => handleDelete("messages", m._id)}
                >
                  <Text style={styles.deleteTxt}>×</Text>
                </TouchableOpacity>
              </View>
